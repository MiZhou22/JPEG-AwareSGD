...
class y_quantize(nn.Module):
    ...
    def __init__(self, rounding, factor=1):
        ...
        self.factor = factor
        self.y_table = DiffJPEG.utils.y_table

    def forward(self, image):
        image = image.float() / (self.y_table * self.factor + 1e-9)
...
class c_quantize(nn.Module):
    ...
    def __init__(self, rounding, factor=1):
        ...
        self.rounding = rounding
        self.factor = factor
        self.c_table = DiffJPEG.utils.c_table

    def forward(self, image):
        image = image.float() / (self.c_table * self.factor + 1e-9)
        image = self.rounding(image)
        return image
...
class compress_jpeg_gray(nn.Module):
    """ Full JPEG compression algortihm
    Input:
        imgs(tensor): batch x 1 x height x width
        rounding(function): rounding function to use
        factor(float): Compression factor
    Ouput:
        compressed(dict(tensor)): batch x h*w/64 x 8 x 8
    """
    def __init__(self, rounding=torch.round, factor=1):
        super(compress_jpeg_gray, self).__init__()
        self.l2 = nn.Sequential(
            block_splitting(),
            dct_8x8()
        )
    ...
    def forward(self, image):
        """
        @param image: the images in NCHW
        @return: the quantization blocks and the results of the DCT and the quantization
        """
        y = image*255
        y = y[:, 0, ...]    # NHW
        components = {'y': y}
